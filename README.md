# Electrical Engineering Portfolio

Hello, my name is Kostas.

I am an Electrical Engineering student with practical experience in industrial automation, electrical design, CAD modeling, and PCB design.

------------------------Skills------------------------

* EPLAN Electric P8
* Siemens TIA Portal
* ZelioSoft/LogoSoft
* Kicad/Altium
* SolidWorks
* Electrical Installations
* PLC Programming

------------------------Portfolio------------------------

This repository contains examples of my work in:

* Electrical schematics and panel design (EPLAN)
* PCB design projects (Kicad/Altium)
* Mechanical CAD models (SolidWorks)
* PLC and automation projects (TIA Portal)
* Relay logic projects (ZelioSoft/LogoSoft)

Currently, project documentation is provided in PDF format. Original project files will be added progressively.

------------------------Practical Experience------------------------

Alongside my studies, I work independently on residential electrical installations and renovation projects.

My experience includes:

* Electrical wiring and rewiring
* Distribution panel installation and modification
* Lighting and power circuits
* Fault diagnosis and troubleshooting
* Residential renovation electrical work
* TV and antenna installation, maintenance
* Compliance with electrical safety practices

This practical experience complements my engineering studies and automation projects by providing hands-on exposure to real-world electrical systems.
## Engineering Activities

------------------------Robotics Club – University of Patras------------------------

Member of the Robotics Club at the University of Patras and part of the Mechatronics Team.
[https://robotics-club.upatras.gr/](https://robotics-club.upatras.gr/)
Activities include:

* Mechanical and electrical system integration
* Robotics and automation projects
* Sensor and actuator implementation
* CAD design and prototyping
* Team-based engineering project development
* Problem solving and system testing

Working within a multidisciplinary engineering team has strengthened my practical skills in mechatronics, automation, design, and technical collaboration.

------------------------Additional Content------------------------

Additional project photos and behind-the-scenes updates can be found on my Instagram: [@Kostassere](https://www.instagram.com/kostassere).

------------------------Contact------------------------

Email: Seresiotko@gmail.com
LinkedIn: www.linkedin.com/in/kostassere
